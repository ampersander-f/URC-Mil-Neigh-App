...
class Grapher:
	...
	def graph(self, title, fn):
		...
		#making it pretty by just saving the data to a file and making a specific graphing program. need for formal presentation at CAURS
		if self.tag == "CAURS":
			f = open("caursarea_2.txt", "w")
			g = open("caurspressure_2.txt", "w")

			for p in self.pressure:
				g.write(str(p[0]) + "," + str(p[1]) + "\n")
			for a in self.area:
				f.write(str(a) + "\n")
			f.close()
			g.close()

			#does it look okay before I put in all the work?
			fig = plt.figure()
			ax = fig.add_subplot(1,1,1)

			ax.plot(self.area, self.pressure)
			plt.savefig("CAURS_pressure")
		...
		#moving average smoothing
			...
			tck_y = interpolate.splrep(x=self.area, y=y, s=666)
		...
		#attempting to use a convolution to best graph the data
